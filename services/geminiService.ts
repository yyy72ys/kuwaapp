...
export const suggestScientificName = async (input: string): Promise<string[]> => {
  if (!API_KEY) {
    return [];
  }

  const prompt = `あなたは世界クラスの昆虫学者で、特に甲虫類に詳しい専門家です。
ユーザーからの部分的な入力に基づき、クワガタ・カブトムシの学名の候補を5つまで提案してください。
入力は日本語（カタカナ・ひらがな）またはラテン語の可能性があります。必ず完全な学名を返してください。

ユーザー入力: "${input}"

レスポンスは、文字列のJSON配列として返してください。例: ["Dorcus hopei binodulosus", "Prosopocoilus giraffa"]
`;
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          items: {
            type: Type.STRING,
            description: "A single scientific name suggestion for a beetle.",
          }
        }
      }
    });
    ...
    console.error("Error generating scientific name suggestions:", error);
    return [];
  }
};
